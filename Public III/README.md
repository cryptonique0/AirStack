...
### ⚡ Top-10 Leaderboard Playbook (Single Commit)
If you need a one-shot push from rank ~300 into the top 10, stack these already-built features in one deployment/commit:
- **Time-lock stake for boost**: Call `createStake(amount)` to activate the 15% boost for 30 days, then `releaseStake()` when done.
- **Link verifiable NFTs**: Use `linkBadge(contract, source)` to add 2-3 whitelisted ERC721s (Talent Protocol, Gitcoin Passport, Polygon ID, or custom) for +30 pts each.
- **Record referrals fast**: Admins call `recordReferral(referrer, referred)` to give the referred user +20 pts and the referrer +20 pts per successful invite.
- **Stay fresh + active**: Add a recent credential (within 90 days) and update your profile to trigger freshness + streak bonuses via `_updateActivityStreak()`.
- **Apply seasonal multipliers**: Admins can `awardSeasonalMultiplier(user, 115-125)` and `advanceSeason()` to lock in the boost for the next season.
- **Refresh cached leaderboard**: Any reputation-changing action triggers `_updateLeaderboard(user)`, so top-100 pagination stays O(limit) and shows your jump immediately.

> Expected lift: ~170–220 points in one cycle (15% stake boost on base score + 2–3 linked badges + 2 referrals + freshness + streak), typically enough to leap from ~300 into the top 10 given current scoring distribution.
